...
function sc_player_weapon_channel_update(_player, _runtime, _weapon_key, _fire, _debug = false, _shot_override = undefined, _firing_override = undefined)
{
    if (!sc_player_weapon_channel_operational(_player, _runtime))
    {
        sc_player_weapon_runtime_release(_runtime);
        return false;
    }

    if (!_player.combat.weapons_allowed || is_undefined(_weapon_key))
    {
        sc_player_weapon_runtime_release(_runtime);
        return false;
    }
    ...
    if (_weapon.delivery.type == AttackDelivery.BEAM)
    {
        if (instance_exists(_runtime.active_delivery_id))
        {
            ...
            var _sustained = sc_beam_sustain(_runtime.active_delivery_id, _muzzle_x, _muzzle_y, _angle);

            if (_sustained)
            {
                sc_audio_weapon_play(_player, _weapon, _muzzle_x, _muzzle_y);
                sc_player_weapon_heat_add(_player, _runtime, _weapon);
            }
    ...
    if (!_fire || GAME_TICK < _runtime.next_fire_tick)
        return false;
    ...
    if (_delayed_burst)
    {
        sc_player_weapon_burst_begin(_player, _runtime, _weapon_key, _weapon, _shot, _muzzle_x, _muzzle_y, _angle);
        _runtime.next_fire_tick = GAME_TICK + max(1, round(_firing.interval / _fire_rate));
    ...
    if (_firing.mount_mode == WeaponMountMode.HARDPOINT)
    {
        ...
        _runtime.hardpoint_cursor = (_runtime.hardpoint_cursor + 1)
            mod array_length(sc_player_hardpoint_group_get(_player, _firing));
    }
...
function sc_player_weapon_heat_channel_update(_player, _runtime)
{
    var _heat = _runtime.heat;
    if (!_heat.enabled) return false;

    var _stats = _player.ship.stats.final;
    var _maximum = max(1, _stats.weapon_heat_maximum);
    var _cooling_effectiveness = sc_ship_system_effectiveness_get(_player.ship, "cooling");

    _heat.current = clamp(_heat.current, 0, _maximum);
    ...
    if (_heat.current > 0 && _cooling_effectiveness > 0)
    {
        _heat.current = max(
            0,
            _heat.current
    ...
    if (_heat.locked && _heat.current <= _maximum * 0.2)
    {
        _heat.locked = false;
        show_debug_message("PLAYER WEAPON GROUP COOLED");
    }
